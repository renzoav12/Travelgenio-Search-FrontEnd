...
const Location: FunctionComponent<Props> = props => {
  ...
  const street = props.location.address.street + (props.location.address.streetNumber.length > 1 ? " " + props.location.address.streetNumber : "");
  const city = (street.length > 1 ? ", ": "") + props.location.address.city;
  const address = <Box className={classes.address}>{street}{city}</Box>;
