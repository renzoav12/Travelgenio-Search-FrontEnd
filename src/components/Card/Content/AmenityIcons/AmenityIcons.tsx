...
const getAmenityIcon = (amenity: Amenity): string | undefined =>
  icons.get(amenity.id);
const getAmenityOrder = (amenity: Amenity): number =>
  orders.get(icons.get(amenity.id) || "") || 99;
...
const AmenityIcons: FunctionComponent<Props> = (props) => {
  ...
  const amenities = props.amenities
    ...
    .sort((amenity, otherAmenity) => {
      return getAmenityOrder(amenity) - getAmenityOrder(otherAmenity);
    })
    .map((amenity) => (
      <img
