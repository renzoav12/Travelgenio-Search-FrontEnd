...
const RangeFilter: FunctionComponent<Props> = (props) => {
  ...
  const [value, setValue] = useState<Array<number>>([
    props.filter.boundaries.min,
    props.filter.boundaries.max,
  ]);
  ...
  useEffect(() => {
    setValue([
      props.filter.value && props.filter.value.min
        ? props.filter.value.min
        : props.filter.boundaries.min,
      props.filter.value && props.filter.value.max
        ? props.filter.value.max
        : props.filter.boundaries.max,
    ]);
  }, [props.filter.value]);
