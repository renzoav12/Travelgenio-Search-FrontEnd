...
import { SearchFetchParameters, SearchFetchFilterParameters } from '../../model/search';

function mapToObject(o, m) {
    for(let[k,v] of m) { o[k] = v }
}

export const searchCreateRequest = (rootState: RootState): SearchFetchParameters => {
    ...
    rootState.search.filters.forEach((filter) => {
        switch(filter.type) {
            ...
            case FilterType.Range:
                ...
                if (!(!rangeFilter.values)) {
                    const rangeProp: RangeProp = rangeFilter.values;
                    if (rangeProp) {
